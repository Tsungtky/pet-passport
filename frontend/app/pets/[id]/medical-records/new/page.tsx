...
export default function NewMedicalRecordPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  return (
    <main style={{ minHeight: "100vh", background: "linear-gradient(135deg, #f5f3ff 0%, #eff6ff 50%, #f0fdf4 100%)", padding: "2rem" }}>
      <div style={{ maxWidth: "520px", margin: "0 auto" }}>
        ...
        <h1 style={{ color: "#1e1b4b", fontWeight: 800, fontSize: "1.75rem", marginBottom: "0.25rem" }}>新增病歷紀錄</h1>
        <p style={{ color: "#94a3b8", fontSize: "0.875rem", marginBottom: "2rem" }}>記錄這次就診的詳細資訊</p>
        ...
        <div style={{ backgroundColor: "rgba(255,255,255,0.85)", backdropFilter: "blur(12px)", borderRadius: "1.5rem", padding: "2rem", boxShadow: "0 4px 20px rgba(0,0,0,0.06)", border: "1px solid rgba(255,255,255,0.6)", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          {[
            { label: "類型", value: recordType, setValue: setRecordType, placeholder: "例：看診、手術、檢查", type: "text" },
            { label: "標題", value: title, setValue: setTitle, placeholder: "例：年度健康檢查", type: "text" },
            { label: "描述", value: description, setValue: setDescription, placeholder: "簡單描述", type: "text" },
            { label: "日期", value: recordDate, setValue: setRecordDate, placeholder: "", type: "date" },
            { label: "診所名稱", value: clinicName, setValue: setClinicName, placeholder: "例：台北動物醫院", type: "text" },
            { label: "獸醫名稱", value: vetName, setValue: setVetName, placeholder: "例：王醫師", type: "text" },
